...
@WebServlet(
		description = "사용자가 웹에서 *.tam 을 했을 경우 이 서블릿이 응답을 해주도록 한다.", 
		urlPatterns = { "*.tam" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:\\git\\realSemi\\realSemi\\src\\main\\webapp\\WEB-INF\\Command.properties", description = "*.tam 에 대한 클래스의 매핑파일")
		})
public class FrontController extends HttpServlet {
	...
	/*
        웹브라우저 주소창에서  *.tam 을 하면 FrontController 서블릿이 응대를 해오는데 
	...
	*/   
		// *** 확인용 *** //
		// System.out.println("확인용 => 서블릿 FrontController 의 init(ServletConfig config) 메소드가 실행되었다. ");
		// http://localhost:9090/MyMVC/hjjjjssudddju.ta 아무거나.tam 하면 콘솔에 뜬다.
		...
		// 확인용 props=>C:/Users/82105/Desktop/쌍용/java파일/NCS/workspace_jsp/MyMVC/src/main/webapp/WEB-INF/Command.properties
		
		try {
			fis = new FileInputStream(props);
			// fis 는 C:/Users/82105/Desktop/쌍용/java파일/NCS/workspace_jsp/MyMVC/src/main/webapp/WEB-INF/Command.properties 파일의 내용을 읽어오기 위한 용도로 쓰이는 객체이다.
			...
			pr.load(fis);
		/*
			pr.load(fis); 은  fis 객체를 사용하여 C:/Users/82105/Desktop/쌍용/java파일/NCS/workspace_jsp/MyMVC/src/main/webapp/WEB-INF/Command.properties 파일의 내용을 읽어다가 
		...
		/*
	        pr.keys(); 은
	        C:/Users/82105/Desktop/쌍용/java파일/NCS/workspace_jsp/MyMVC/src/main/webapp/WEB-INF/Command.properties 파일의 내용물에서 
	    ...
	    */
			...
			while(en.hasMoreElements()) {
				...
				// 확인용 key => /test/test2.tam
				// 확인용 key => /test1.tam
				
				//System.out.println("### 확인용 value => " + pr.getProperty(key));
				// ### 확인용 value => test.controller.Test2.Controller
				...
				if(className != null) {
					...
					//System.out.println("확인용 obj => "+ obj.toString());
			...
			System.out.println(">>> 문자열로 명령되어진 클래스가 존재하지 않습니다.");
			e.printStackTrace();
		} catch(Exception e) {
			e.printStackTrace();
		}
	...
	//	System.out.println("확인용 url =>" + url);
		// 확인용 url => http://localhost:9090/MyMVC/member/idDuplicateCheck.tam
		
		String uri = request.getRequestURI();  
	//	System.out.println("확인용 uri =>" + uri);
		// 확인용 uri =>/MyMVC/member/idDuplicateCheck.tam
		// 확인용 uri =>/MyMVC/test1.tam
		// 확인용 uri =>/MyMVC/test/test2.tam
		
		String key = uri.substring(request.getContextPath().length());//   /MyMVC
		/*
			/member/idDuplicateCheck.tam
			/test1.tam
			/test/test2.tam
		*/
		
		AbstractController action = (AbstractController)cmdMap.get(key);
		// AbstractController 가 부모클래스가 AbstractController로 형변환을 할 수 있다..?
		
		if(action == null) {
			System.out.println(">>> " + key + " 은 URI 패턴에 매핑된 클래스는 없습니다. <<<");
			// >>> /member/idDuplicateCheck.tam 은 URI 패턴에 매핑된 클래스는 없습니다. <<<
		}
		else {
			try {
				action.execute(request, response);
				boolean bool = action.isRedirect();
				String viewPage = action.getViewPage(); // test1,2에서 set을 해주었기에 get으로 받아준다.
