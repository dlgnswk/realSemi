...
let b_telCheck = true;
// 연락처 인증이 완료됐는지 확인하기위한 용도 (기본값: 이미 인증되어있기 때문에 true)
...
$(document).ready(function() {
	...
	const regExp_pwd = new RegExp("^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,20}$");
	
	// 아이디가 pwd인 것의 포커스를 잃어버렸을 경우(blur) 이벤트를 처리해주는 것
	// 비밀번호 유효성 검사 후 잘못 됐을 시, 오류 메시지 및 빨간 보더
   ...
   let b_btnChgMobileClick = false;
   
   // 연락처 "변경하기" 버튼 클릭 시
   $("button#btn_chg_mobile").bind("click", function (e) {
	   
	   // "인증하기" 버튼으로 변경 후, 연락처 변경가능하도록 readonly 해제
       $(e.target).html("인증하기");
   	   $("input#mobile").prop("readonly",false);
   	   b_btnChgMobileClick = true;
	   
	   if(b_btnChgMobileClick) {
...
/*   if(!b_telCheck) {
	 ...
	 const val_email = $("input#email").val();
	 
	 if ( regExp_email.test(val_email) ) {
     ...
     // 이메일, 이름, 생년월일 유효성 검사 통과시  true 반환  == 끝 == //
     
     
     // 유효성 false 시, 빨간 보더 == 시작 == //
      
   	 if(b_emailCheck == false) {
		$("input#email").parent().css("border","1px solid #d12b2b");
		showBanner(); // 공백있으면 빨간 바 보여주고 리턴
   ...
   if(b_emailCheck && b_nameCheck && b_birthCheck && b_requiredInfo && b_requiredAgree && b_telCheck) {
...
function resendMsgTel() {
	...
	const mobile = $("input#mobile").val();
	const result = confirm(mobile+" 번호로 인증번호를 재발송 하시겠습니까?");
		...
		if(!result){
			...
			alert(mobile+" 번호로 문자 전송을 완료했습니다.\n인증번호를 입력하여주세요.")
			 /*
		    $.ajax({
			  url:"smsSend.tam",
			  type:"post",
